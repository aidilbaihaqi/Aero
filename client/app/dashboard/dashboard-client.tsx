...
export function DashboardClient({ initialStats, initialFares, initialChartData }: DashboardClientProps) {
    ...
    // Using SWR for data fetching and caching, fallback to initialData (SSR)
    const { data: stats, mutate: mutateStats, isLoading: isStatsLoading } = useSWR<DashboardStats>("/api/stats/dashboard", fetcher, { fallbackData: initialStats || undefined });
    const { data: fares, mutate: mutateFares, isLoading: isFaresLoading } = useSWR<FlightFare[]>("/api/flights/history?limit=6", fetcher, { fallbackData: initialFares });
                        ...
                        <CardGlassHeader>
                            ...
                            <div className="mt-4">
                                ...
                                <div className="mt-1 font-display text-2xl font-bold">
                                    {!stats ? "—" : formatShortPrice(stats.avg_price ?? 0)}
                                </div>
