...
export function HistoryClient({ initialRuns }: HistoryClientProps) {
  const { data: runs = [], isLoading } = useSWR<ScrapeRun[]>("/api/flights/runs?limit=100", fetcher, { fallbackData: initialRuns });

  const loading = isLoading && runs.length === 0;
