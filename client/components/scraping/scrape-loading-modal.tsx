    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="relative w-full max-w-md overflow-hidden rounded-3xl bg-white shadow-2xl transition-all p-0 mx-4">
        ...
        <div className="h-32 bg-black relative overflow-hidden">
          {/* Clouds / Decor */}
          <div className="absolute top-10 left-10 h-8 w-20 bg-white/10 rounded-full blur-xl"></div>
          <div className="absolute top-5 right-20 h-12 w-32 bg-white/10 rounded-full blur-xl"></div>

          {/* Plane Animation Container */}
          {isLoading && (
            <div className="absolute top-1/2 left-0 w-full -translate-y-1/2">
              <div className="relative w-full h-12">
                <Plane className="absolute text-white h-8 w-8 animate-[fly_3s_linear_infinite]" style={{ filter: "drop-shadow(0 4px 6px rgba(255,255,255,0.2))" }} />
              </div>
            </div>
          )}
        </div>
        ...
        <div className="px-6 pb-6 pt-10 text-center relative">
          ...
          {/* Error Message */}
          {!isLoading && error && (
            <div className="w-full mb-6 rounded-xl bg-neutral-50 p-3 text-left border border-neutral-200 flex gap-3">
              <AlertCircle className="h-5 w-5 text-neutral-900 shrink-0" />
              <div className="text-xs text-neutral-700">{error}</div>
            </div>
          )}
